<table style="border-style: none">
  <tr style="border-style: none">
    <td style="border-style: none"><img src="./docs/images/logo.png" alt="anonymongo logo" width="120" align="right" /></td>
    <td style="border-style: none">
<p><h1>anonymongo</h1></p>
        <p>
...
* **Eager redaction**: Redact field names from specified namespaces for dynamic schemas that contain sensitive values in
  field names.
...
* **Complete insights preservation**: The logs will be redacted of any sensitive information, but the analysis of the
  log file will remain intact.
* Change the default redaction placeholder string (default: `"REDACTED"`)
* **Read logs from Atlas Clusters**: Redact logs straight from MongoDB Atlas clusters. This feature requires an Atlas
  API key (see [Atlas Administration API](https://www.mongodb.com/docs/atlas/api/atlas-admin-api/)).
...
- [anonymongo](#anonymongo)
    ...
    - [Issues section of this repository.](#issues-section-of-this-repository)
    - [5. Disclaimer](#5-disclaimer-1)
...
Having trouble running `anonymongo` after installing it with Homebrew?
See [4. Adding anonymongo to trusted software on macOS](#4-adding-anonymongo-to-trusted-software-on-macos).
...
Extract the downloaded archive and run the `anonymongo` binary. Depending on the OS settings, you may be prompted to
trust the program explicitly.
...
To use this feature, you need to provide a key using the `--encryptionKey` flag. The key must be a 32-byte
...
3. Go to ***System Settings*** -> ***Privacy & Security***, and click the ***Open Anyway*** button. You will be prompted
   again - click
   ***Open Anyway*** again.
